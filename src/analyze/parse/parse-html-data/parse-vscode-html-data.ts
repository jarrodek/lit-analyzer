...
function tagDataToHtmlTag(tagData: ITagData, config: ParseVscodeHtmlDataConfig): HtmlTag {
  ...
  const descriptionResult = stringOrMarkupContentToString(description)

  return {
    tagName: name,
    ...(descriptionResult ? { description: descriptionResult } : {}),
    attributes,
    events,
    properties: [],
    slots: [],
...
function valueSetsToTypeMap(valueSets: IValueSet[]): Map<string, SimpleType> {
  const entries = valueSets.map(
    (valueSet) => [valueSet.name, attrValuesToUnion(valueSet.values)] as [string, SimpleType]
  )
