...
export function documentationForCssPart(cssPart: HtmlCssPart, _options: DescriptionOptions = {}): string | undefined {
  const relatedText = (() => {
...
export function mergeHtmlTags(tags: HtmlTag[]): HtmlTag[] {
  ...
  for (const tag of tags) {
    ...
    if (existingTag != null) {
      const builtIn = tag.builtIn || existingTag.builtIn
      const global = tag.global || existingTag.global
      const declaration = tag.declaration || existingTag.declaration
      const description = tag.description || existingTag.description
