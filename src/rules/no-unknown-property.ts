  ...
  visitHtmlAttribute(htmlAttr, context) {
    ...
    if (htmlAttrTarget == null) {
      ...
      context.report({
        ...
        fixMessage:
          suggestedMemberName == null ? undefined : `Did you mean '${suggestedModifier}${suggestedMemberName}'?`,
        suggestion,
        fix:
...
function getSuggestionText({
  ...
  const tagIsFromLibrary =
    iterableFirst(definitionStore.getDefinitionForTagName(htmlTag.tagName)?.identifierNodes)?.getSourceFile()
